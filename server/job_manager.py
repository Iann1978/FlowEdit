"""Job management for tracking FlowEdit jobs."""
...
class JobManager:
    """Manages FlowEdit job state and tracking."""
    
    def __init__(self, jobs_dir: Optional[Path] = None):
        """Initialize job manager.
        
        Args:
            jobs_dir: Directory to store job data (default: from config)
        """
        if jobs_dir is None:
    ...
    def cleanup_job(self, job_id: str) -> bool:
        ...
        - UPLOADING/PENDING: Full cleanup (delete directory and remove from memory)
        - RUNNING: Update status to CANCELLED (keep directory/memory for logs/results)
        ...
        # Full cleanup for incomplete jobs
        if status in [JobStatus.UPLOADING, JobStatus.PENDING]:
        ...
        # Cancel active jobs (keep directory/memory for logs/results)
